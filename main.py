...
def check_pattern_func(text):
    ...
    print(tag)
    if tag == 'Россия.Москва':
        return bool([word for word in words if [True for key_word in key_words if key_word in word]])

    elif tag in ['Россия.Происшествия','Из жизни.Происшествия','Силовые структуры.Криминал','Россия.Общество','Дом.Квартира','Дом.Город','Дом.Офис','Интернет и СМИ.Интернет','Интернет и СМИ.Coцсети']:
        for word in words:
            if (('моск' in word) and (len(word) < 8)) or (('столиц' in word) and (len(word) < 10)) or (('столич' in word) and (len(word) < 10)):
                return bool([word for word in words if [True for key_word in key_words if key_word in word]])
    return False
